...
public class TileAttacher {
	...
	//sets portals as neighbours
	protected void setPortalTilesAsNeighbours()
	{
		for (int row =0; row < GAME_BOARD_DIMENSION; row++ )
		{
			for (int col =0; col < GAME_BOARD_DIMENSION; col++)
			{
				if (tileList[col][row] instanceof Portal)
				{
					Portal p = (Portal) tileList[col][row];
					Tile t = p.getAttachedPortal();
					Portal p2 = (Portal) t;
					p.setAttachedPortal(p2);
					p2.setAttachedPortal(p);
				}
			}
