...
import model_gameboard.IceWallMelter;
...
public class ObjectMover {
	...
	public boolean moveTurtle(RobotTurtle rt, Card c)
	  {
		  ...
		  if (c.equals(cardDeck[1])) //if card = step forward
		  {
			  ...
			  if (rtpc.checkIfTileAheadExists()) //making sure robot turtle isnt trying to go off the gameboard
			  {
				  ...
				  if (moveChecker.checkIfTurtleCanMove(rtpc.calculateNewPosition(), rt.getDirection())) //checking if obstacle blocking turtle's path
				  {
					  ...
					  if (checkIfTeleported(turtleNewPos, turtleUpdater, rt ))
					  {
						  return true;
		  ...
		  else if (c.equals(cardDeck[0]))// turn left card
		  {
			  ...
			  int[] pos = { rt.getXPos(), rt.getYPos()};
			  checkIfTeleported(pos, turtleUpdater, rt);
		  }
		  ...
		  else if (c.equals(cardDeck[4])) //laser card
		  {
			  IceWallMelter melter = new IceWallMelter();
			  melter.meltIceWallsAhead(rt.getDirection(), rt.getXPos(), rt.getYPos(), gameBoard);
		  }
	...
	protected boolean checkIfTeleported(int[] turtleNewPos, RobotTurtlePositionUpdater turtleUpdater, RobotTurtle rt)
	{
		if (gameBoard.getTileList()[turtleNewPos[0]][turtleNewPos[1]].getTileType() == "Portal")
		  {
			  turtleUpdater.teleport(turtleNewPos[0], turtleNewPos[1], rt);
			  return true;
		  }
		return false;
	}
